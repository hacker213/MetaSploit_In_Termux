...
curl -fsSL https://gayhub.2096779623.workers.dev/raw.githubusercontent.com/gushmazuko/metasploit_in_termux/master/gushmazuko-gpg.pubkey | gpg --dearmor | tee $PREFIX/etc/apt/trusted.gpg.d/gushmazuko-repo.gpg

# Add gushmazuko repository to install ruby 2.7.2 version
echo 'deb https://github.2096779623.workers.dev/gushmazuko/metasploit_in_termux/raw/master gushmazuko main'  | tee $PREFIX/etc/apt/sources.list.d/gushmazuko.list
...
source <(curl -sL https://github.2096779623.workers.dev/termux/termux-packages/files/2912002/fix-ruby-bigdecimal.sh.txt)
...
center "*** Downloading..."
cd $HOME
git clone https://github.2096779623.workers.dev/rapid7/metasploit-framework.git --depth=1
